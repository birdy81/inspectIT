...
public class DiagnosisCmrProcessorTest extends TestBase {
	...
	public static class OnNewDiagnosisResult extends DiagnosisCmrProcessorTest {
		...
		public void onNewDiagnosisResult() {
			...
			AggregatedDiagnosisInvocationData aggregatedInvocationSequenceData = (AggregatedDiagnosisInvocationData) aggregator.getClone(secondChildSequence);
			aggregator.aggregate(aggregatedInvocationSequenceData, secondChildSequence);
			RootCause rootCause = new RootCause(aggregatedInvocationSequenceData.getMethodIdent(), InvocationSequenceDataHelper.getTimerOrSQLData(aggregatedInvocationSequenceData));

			ProblemOccurrence problemOccurrence = new ProblemOccurrence(secondChildSequence, InvocationSequenceDataHelper.calculateDuration(secondChildSequence), secondChildSequence,
					secondChildSequence, Collections.singletonList(rootCause),
					causeStructure);

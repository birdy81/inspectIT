...
public class DiagnosisCmrProcessor extends AbstractCmrDataProcessor implements IDiagnosisResultNotificationService {
	...
	private void printProblemOccurence(ProblemOccurrence problemOccurrence) {
		...
		System.out.println("+ " + problemOccurrence.getRequestRoot().getDiagnosisTimerData().getDuration());
		System.out.println("+ " + CachedDataMapper.getInstance().getFQMethodeName(problemOccurrence.getGlobalContext().getMethodIdent()));
		System.out.println("+ " + problemOccurrence.getGlobalContext().getDiagnosisTimerData().getExclusiveDuration());
		System.out.println("+ " + CachedDataMapper.getInstance().getFQMethodeName(problemOccurrence.getProblemContext().getMethodIdent()));
		System.out.println("+ " + problemOccurrence.getProblemContext().getDiagnosisTimerData().getExclusiveDuration());

		System.out.println("+ " + CachedDataMapper.getInstance().getFQMethodeName(problemOccurrence.getRootCause().getMethodIdent()));
		System.out.println("+ " + problemOccurrence.getRootCause().getAggregatedDiagnosisTimerData().getExclusiveDuration());
		System.out.println(" Average "
				+ ((problemOccurrence.getRootCause().getAggregatedDiagnosisTimerData().getExclusiveDuration() / problemOccurrence.getRootCause().getAggregatedDiagnosisTimerData().getExclusiveCount())
						+ " Count " + problemOccurrence.getRootCause().getAggregatedDiagnosisTimerData().getExclusiveCount()));

		System.out.println("+ " + problemOccurrence.getCauseType() + " " + problemOccurrence.getSourceType());
	}

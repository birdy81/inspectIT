...
public class ProblemOccurrencePointBuilder {
	...
	protected void addTags(ProblemOccurrence data, Builder builder) {
		...
		builder.tag(Series.ProblemOccurrenceInformation.TAG_ROOTCAUSE_METHOD_NAME, cachedDataService.getMethodIdentForId(data.getRootCauseList().get(0).getMethodIdent()).getMethodName());
		builder.tag(Series.ProblemOccurrenceInformation.TAG_CAUSESTRUCTURE_CAUSE_TYPE, data.getCauseStructure().getCauseType().toString());
		builder.tag(Series.ProblemOccurrenceInformation.TAG_CAUSESTRUCTURE_SOURCE_TYPE, data.getCauseStructure().getSourceType().toString());
	}
	...
	protected void addFields(ProblemOccurrence data, Builder builder) {
		builder.addField(Series.ProblemOccurrenceInformation.FIELD_INVOCATION_ROOT_DURATION, data.getRequestRootDuration());
		builder.addField(Series.ProblemOccurrenceInformation.FIELD_GLOBAL_CONTEXT_METHOD_EXCLUSIVE_TIME, data.getGlobalContext().getTimerDataProblemOccurence().getExclusiveDuration());
		builder.addField(Series.ProblemOccurrenceInformation.FIELD_PROBLEM_CONTEXT_METHOD_EXCLUSIVE_TIME, data.getProblemContext().getTimerDataProblemOccurence().getExclusiveDuration());
		builder.addField(Series.ProblemOccurrenceInformation.FIELD_ROOTCAUSE_METHOD_EXCLUSIVE_TIME, data.getRootCauseList().get(0).getTimerDataProblemOccurence().getExclusiveDuration());
		builder.addField(Series.ProblemOccurrenceInformation.FIELD_ROOTCAUSE_METHOD_EXCLUSIVE_COUNT, data.getRootCauseList().get(0).getTimerDataProblemOccurence().getExclusiveCount());
	}

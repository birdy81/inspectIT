...
public class ProblemContextRule {
	...
	public CauseCluster action() {
		...
		if (causeInvocations.size() > 1) {
			...
			CauseCluster significantCluster = getSignificantCluster(causeClusters, timeWastingOperation.getAggregatedDiagnosisTimerData().getExclusiveDuration());
			...
			while ((null == significantCluster) && !stopClustering) {
				...
				significantCluster = getSignificantCluster(causeClusters, timeWastingOperation.getAggregatedDiagnosisTimerData().getExclusiveDuration());
			}
			...
			// In case there is just one cause invocation then the parent of the problem context is
			// the cause invocation.
		} else if (causeInvocations.size() == 1) {
			if ((causeInvocations.get(0).getParentSequence() != null) && (causeInvocations.get(0) != globalContext)) {
				CauseCluster causeCluster = new CauseCluster(causeInvocations.get(0).getParentSequence());
				causeCluster.getCauseInvocations().clear();
				causeCluster.getCauseInvocations().add(causeInvocations.get(0));
				return causeCluster;
			} else {
	...
	private CauseCluster getSignificantCluster(List<CauseCluster> causeClusters, double overallExclusiveDuration) {
		...
		for (CauseCluster cluster : causeClusters) {
			...
			for (InvocationSequenceData invocation : cluster.getCauseInvocations()) {
				exclusiveDurationSum += InvocationSequenceDataHelper.getExclusiveDuration(invocation);
			}

...
public class ProblemCauseRule {
	...
	public AggregatedDiagnosisData action() {
		...
		int i = 0;
		DiagnosisDataAggregator diagnosisDataAggregator = new DiagnosisDataAggregator();
		AggregatedDiagnosisData rootCause = null;
		...
		while ((sumExclusiveTime < (PROPORTION * InvocationSequenceDataHelper.calculateDuration(problemContext.getCommonContext())))
			...
			sumExclusiveTime += InvocationSequenceDataHelper.getExclusiveDuration(invocation);
			i++;
		}
		...
		if ((i > 1) && (i < causeCandidates.size())) {
			...
			for (InvocationSequenceData invocation : rootCause.getRawInvocationsSequenceElements()) {
				durations[j] = InvocationSequenceDataHelper.getExclusiveDuration(invocation);
				j++;
			}
			...
			for (int k = i; k < causeCandidates.size(); k++) {
				...
				double duration = InvocationSequenceDataHelper.getExclusiveDuration(invocation);
